...
import type { DNASequence, BlastResults, BlastHit, BlastRequest } from "../types/bioinformatics.js";

export class BlastApiClient {
  private readonly baseUrl = "https://blast.ncbi.nlm.nih.gov/Blast.cgi";
  private readonly statusCheckInterval = 5000; // 5 seconds between status checks
  private readonly maxWaitTime = 300000; // 5 minutes max wait
   ...
   */
  public async checkStatus(rid: string): Promise<'WAITING' | 'READY' | 'UNKNOWN'> {
    try {
   ...
   */
  private buildBlastParams(sequence: string): string {
    const params = new URLSearchParams({
      CMD: 'Put',
   ...
   */
  private extractRID(response: string): string | null {
   ...
   */
  private parseBlastResults(rid: string, data: any): BlastResults {
    ...
    try {
      ...
      results.hits = hits.slice(0, 10).map((hit: any) => this.parseBlastHit(hit))
                        .filter((hit: BlastHit | null) => hit !== null) as BlastHit[];
   ...
   */
  private parseBlastHit(hitData: any): BlastHit | null {
    try {
   ...
   */
  private extractSpeciesNames(description: string): { scientificName: string; commonName?: string } {
   ...
   */
  private calculateIdentity(identities: number, alignmentLength: number): number {
   ...
   */
  private calculateCoverage(alignmentLength: number, queryFrom: number, queryTo: number): number {
...
export class BlastRateLimiter {
  private requestQueue: BlastRequest[] = [];
  private isProcessing = false;
  private readonly maxRequestsPerMinute = 6; // NCBI allows ~10 seconds between requests
   ...
   */
  public async queueRequest(request: BlastRequest): Promise<BlastResults> {
