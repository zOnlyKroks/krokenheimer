...
export class BlastApiClient {
    ...
    private async runNCBIBlastWithProperFormatting(seq: string): Promise<BlastResults> {
    ...
    private async pollForNCBIResults(rid: string, seq: string): Promise<BlastResults> {
    ...
    private async fetchNCBIResults(rid: string, seq: string): Promise<BlastResults> {
    ...
    private parseBlastJSON(rid: string, seq: string, data: any): BlastResults {
        try {
    ...
    private parseHitsFromJSON(hits: any[], seq: string): BlastHit[] {
        const parsedHits: BlastHit[] = [];
    ...
    private parseTextResults(rid: string, seq: string, text: string): BlastResults {
        const hits: BlastHit[] = [];
    ...
    private parseXMLResults(rid: string, seq: string, xml: string): BlastResults {
        const hits: BlastHit[] = [];
        const seenSpecies = new Set<string>(); // Track species we've already added
        ...
        for (const hitBlock of hitBlocks.slice(0, 15)) { // Check more hits to find unique species
            try {
                ...
                // @ts-ignore
                if (speciesMatch) scientificName = speciesMatch[1];

                // @ts-ignore
                const commonMatch = description.match(/\[([^\]]+)\]/);
                // @ts-ignore
                if (commonMatch) commonName = commonMatch[1];
    ...
    private async runNCBIBlastSimplified(seq: string): Promise<BlastResults> {
    ...
    public async runEBIBlast(sequence: DNASequence): Promise<BlastResults> {
        const seq = sequence.cleaned || sequence.raw;
        ...
        try {
            ...
            for (let i = 0; i < 40; i++) {
                ...
                const statusResponse = await fetch(
                    `https://www.ebi.ac.uk/Tools/services/rest/ncbiblast/status/${jobId}`
                );
                ...
                if (statusResponse.ok) {
                    ...
                    if (status === "FINISHED") {
                        ...
                        const resultResponse = await fetch(
                            `https://www.ebi.ac.uk/Tools/services/rest/ncbiblast/result/${jobId}/out`
                        );
    ...
    private parseEBITextResults(jobId: string, seq: string, text: string): BlastResults {
        const hits: BlastHit[] = [];
        const lines = text.split('\n');
        let currentHit: any = null;
