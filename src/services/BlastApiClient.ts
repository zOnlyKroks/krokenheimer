...
export class BlastApiClient {
    ...
    public async submitSequence(sequence: DNASequence): Promise<string> {
        ...
        if (!match) throw new Error("No RID returned from BLAST");
    ...
    public async checkStatus(
        ...
        for (let attempt = 1; attempt <= this.statusCheckRetries; attempt++) {
            try {
                ...
                if (text.includes("FAILED")) throw new Error("BLAST job failed");
            } catch (err) {
                if (attempt === this.statusCheckRetries) return "UNKNOWN";
    ...
    private async parseBlastResults(
        ...
        results.hits = hits
            .slice(0, 10)
            .map((h: any) => this.parseBlastHit(h))
