import type { DNASequence, ExtractionResult, ValidationResult, AnalysisOptions } from "../types/bioinformatics.js";
...
export class SequenceDetector {
  ...
  private characterAliases: Map<string, string> = new Map([
    // German umlauts
    ['ä', 'ae'], ['ö', 'oe'], ['ü', 'ue'],
    ['Ä', 'AE'], ['Ö', 'OE'], ['Ü', 'UE'],
    // Accented vowels
   ...
   */
  public extractSequencesFromMessage(text: string, options?: Partial<AnalysisOptions>): ExtractionResult {
   ...
   */
  private extractSequentialATCG(text: string): string {
   ...
   */
  private extractWordBasedATCG(text: string): string {
    const words = text.toUpperCase().split(/\s+/);
    let sequence = '';

    for (const word of words) {
      // Skip very short words and obvious non-biological content
      if (word.length < 2 || this.isNonBiological(word)) continue;
   ...
   */
  private extractContinuousSequences(text: string): string[] {
   ...
   */
  private countATCGLetters(text: string): number {
    const cleanText = text.toUpperCase();
    let count = 0;
   ...
   */
  private isValidSequence(sequence: DNASequence, options: AnalysisOptions): ValidationResult {
    const errors: string[] = [];
   ...
   */
  private calculateComplexity(sequence: string): number {
   ...
   */
  private isRepetitive(sequence: string): boolean {
   ...
   */
  private calculateExtractionConfidence(sequence: string, sourceText: string, method: string): number {
    ...
    // Length adjustments
    if (sequence.length >= 20) confidence += 0.1;
    if (sequence.length >= 50) confidence += 0.1;
    if (sequence.length < 10) confidence -= 0.2;
   ...
   */
  private containsBiologicalKeywords(text: string): boolean {
   ...
   */
  private isNonBiological(word: string): boolean {
   ...
   */
  private applyCharacterAliases(text: string): string {
   ...
   */
  private convertIUPACToStandard(sequence: string): string {
   ...
   */
  public setCharacterAlias(from: string, to: string): void {
   ...
   */
  public removeCharacterAlias(from: string): boolean {
   ...
   */
  public getCharacterAliases(): Map<string, string> {
   ...
   */
  public resetCharacterAliases(): void {
   ...
   */
  private removeDuplicates(sequences: DNASequence[]): DNASequence[] {
   ...
   */
  private calculateValidationScore(details: any): number {
    const weights = {
      lengthCheck: 30,
      gcContentCheck: 20,
