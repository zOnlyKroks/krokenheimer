...
export class DownCheckerPlugin implements BotPlugin {
    name = "DownCheckerPlugin";
    description = "Monitor websites and check if they're down";
    version = "1.0.0";

    private logger = new Logger();
    private monitoredSites = new Map<string, MonitoredWebsite>();
    ...
    private async addMonitoring(message: Message, args: string[]): Promise<void> {
        if (args.length === 0) {
        ...
        const monitoredSite: MonitoredWebsite = {
            // @ts-ignore
            url,
    ...
    private async removeMonitoring(message: Message, args: string[]): Promise<void> {
        if (args.length === 0) {
    ...
    private async checkStatus(message: Message, args: string[]): Promise<void> {
        if (args.length === 0) {
    ...
    private async listMonitored(message: Message, args: string[]): Promise<void> {
    ...
    private async checkWebsite(url: string): Promise<{status: 'up' | 'down', responseTime?: number, error?: string}> {
    ...
    private async sendStatusNotification(site: MonitoredWebsite, result: {status: 'up' | 'down', responseTime?: number, error?: string}, previousStatus: string): Promise<void> {
        if (!this.client) return;

        try {
            const channel = await this.client.channels.fetch(site.channelId) as TextChannel;
            if (!channel) return;
    ...
    private generateSiteKey(url: string): string {
