...
export class GifPlugin implements BotPlugin {
  name = "GifPlugin";
  ...
  private async createGif(message: Message, args: string[]): Promise<void> {
...
\`!gif --delay=200 --quality=5\` (attach images)
  ...
  private parseGifOptions(args: string[]): GifOptions {
  ...
  private async createGifFromUrls(urls: string[], options: GifOptions): Promise<string> {
  ...
  private calculateDimensions(firstImage: any, options: GifOptions): { width: number; height: number } {
