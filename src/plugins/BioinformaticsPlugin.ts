...
export class BioinformaticsPlugin implements BotPlugin {
  ...
  async initialize(client: Client, bot: ExtensibleBot): Promise<void> {
    ...
    this.logger.info(
      "BioinformaticsPlugin initialized - automatic DNA sequence detection active",
    );
  }
  ...
  private async scanMessage(message: Message): Promise<void> {
    ...
    try {
      const extractionResult =
        this.sequenceDetector.extractSequencesFromMessage(
          message.content,
      ...
      const bestSequence = extractionResult.sequences.sort(
  ...
  private async processSequence(
    ...
    try {
      ...
      try {
        ...
        const finalEmbed =
          await SequenceFormatter.createSimpleAnalysisEmbed(result);
        ...
        this.logger.error(
          `[BIOINFORMATICS] Automatic analysis failed for ${sequence.cleaned?.length || sequence.raw?.length}bp sequence:`,
          {
            error: errorMessage,
            sequence:
              sequence.cleaned?.substring(0, 50) ||
        ...
        try {
          ...
          this.logger.error(
            "[BIOINFORMATICS] Failed to delete notification message:",
            deleteError,
          );
        }
      }
      ...
      this.logger.error(
        `[BIOINFORMATICS] Sequence processing error (automatic):`,
        {
      ...
      .addFields([
        {
          name: "🔧 Settings",
          value:
            `**Min Length**: ${this.analysisOptions.minSequenceLength} bp\n` +
  ...
  private async setAlias(message: Message, args: string[]): Promise<void> {
    if (args.length !== 2) {
      await message.reply(
        "❌ Usage: `!alias set <character> <replacement>`\n\nExample: `!alias set ä ae`",
      );
      return;
    }
    ...
    if (!fromChar || !toReplacement)
      throw new Error("From or too chart empty!");

    // Validate input
    if (fromChar.length !== 1) {
      await message.reply(
        "❌ The character to replace must be exactly one character.",
      );
      return;
    }
    ...
    if (toReplacement.length > 5) {
      await message.reply(
        "❌ Replacement string cannot be longer than 5 characters.",
      );
      return;
    }
    ...
    if (["A", "T", "C", "G", "a", "t", "c", "g"].includes(fromChar)) {
      await message.reply(
        "❌ Cannot create aliases for standard DNA bases (A, T, C, G).",
      );
      return;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x00ff00)
      .setDescription(
        `Character **${fromChar}** will now be replaced with **${toReplacement}** before DNA sequence processing.`,
      )
      .addFields([
        {
          name: "Example",
  ...
  private async removeAlias(message: Message, args: string[]): Promise<void> {
    ...
    if (!wasRemoved) {
      await message.reply(
        `❌ No alias found for character **${charToRemove}**.`,
      );
      return;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0xff9900)
      .setDescription(
        `Alias for character **${charToRemove}** has been removed.`,
      )
      .setTimestamp();
  ...
  private async listAliases(message: Message): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x0099ff)
      .setDescription(
        `${aliases.size} character alias${aliases.size === 1 ? "" : "es"} configured`,
      )
      .setTimestamp();
  ...
  private async resetAliases(message: Message): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0xff9900)
      .setDescription(
        "All character aliases have been reset to default values.",
      )
      .addFields([
        {
